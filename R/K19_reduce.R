...
# We've already learned the first function from the purrr family: map().
# Now we'll learn the next two: in this koan, we'll focus on reduce(),
# and in koan 20, we'll learn accumulate().
...
# element of `.x` *sequentially* to get you *one* final output. That is, it uses
# `.f` to reduce `.x`.
...
# The first example of how `reduce()` can be used is the function `max()`.
# `max()` takes a vector of numbers and returns the number which is the largest.
...
# I'll write a 2-argument function that takes two numbers, compares them,
# and returns the larger one. I'll call this function "larger", and I'll
# use if_else() to do it:
...
# 2. Test that the function works by filling in the blank below
# with any number to make the statement return TRUE:
...
# Using `reduce()` does the same thing as what I've written above to sequentially
# apply `.f` until `.x` is through:

reduce(c(4, 2, 6, 8), larger)

# So I can write a function my_max() that takes any vector of any length and finds
# what the maximum is by sequentially applying `larger`:
...
# Instead of creating the functions `larger` and `my_max`, if you
# just wanted to find the maximum of a vector using reduce(), you
# can use the formula syntax just like with map(). Since .f is a
# 2-argument function for reduce(), you'll refer to `.x` AND `.y`.
# .x is the value passed from previous evaluations of .f, and .y
# refers to the new element of the vector.
...
# 3. Your turn: create a function `my_min` that finds the minimum
# of a vector of numbers, using `<`, `if_else`, and `reduce`.
...
# So we've learned that you can think of max() as a reduced `>` and
# min() as a reduced `<`. You can also think of sum() as a reduced `+`.
