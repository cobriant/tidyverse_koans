...
# intermediate results so that the output is the same length as `.x`, just
# like map(). The last element of the accumulate() output will be the same
# as the reduce() output.

# This function shows you the computations from the max() algorithm
# we discussed in the previous koan. Run this code to see the result:

accumulate(c(1, 3, 9, 2), ~ if_else(.x > .y, .x, .y))

# The first element of the accumulate() output from above is a 1.
# `.f` hasn't been applied yet. In order to make the output the same length
# as the input, accumulate() just outputs the first element of `.x` as-is.

# The second element of the accumulate output is 3. That's because
# the maximum of 1 and 3 is 3.

# The third element of the accumulate output is 9. That's because
# the maximum of 3 and 9 is 9.

# The fourth element of the accumulate output is 9. That's because
# the maximum of 9 and 2 is 9.
...
# Let's visualize the random walk! Run the code below a couple of times
# to see that random walks aren't more likely to trend downward or upward.
# Time series like stocks or housing prices (whenever the data generating
# process includes a lot of speculation) are modeled well with random walks.
# That's because the best guess anyone can make about a stock's value next period
# is the stock's value this period. If we could do better with our prediction,
# we could make a lot of money. But the point is, nobody can!
...
# Notice that the first difference will recover the vector u (without
# the first term). This happens because:
...
# Wow! You've finished all 20 tidyverse koans, what a wonderful
# accomplishment. You've worked hard and learned so much.

# You may not feel like you've achieved tidyverse enlightenment yet, but
# as you keep practicing, you may find yourself surprised to learn just
# how powerful these concepts truly are. And you may discover you are a
# much more powerful programmer than you ever thought you could be!
